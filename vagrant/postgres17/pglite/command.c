...
static backslashResult exec_command_readonly(PsqlScanState scan_state, bool active_branch);
			 ...
			 PQExpBuffer previous_buf)
{
	...
	if (strcmp(cmd, "a") == 0)
		...
		status = exec_command_readonly(scan_state, active_branch);
...
/*
 * \readonly -- enable/disable readonly action
 */
static backslashResult
exec_command_readonly(PsqlScanState scan_state, bool active_branch)
{
	bool		success = true;
	...
	if (active_branch)
	{
		...
		if (opt)
			success = ParseVariableBool(opt, "\\readonly", &pset.readonly);
		else
			pset.readonly = !pset.readonly;
		if (!pset.quiet)
		{
			if (pset.readonly)
				puts(_("readonly is on."));
			else
				puts(_("readonly is off."));
		}
		free(opt);
	}
	else
